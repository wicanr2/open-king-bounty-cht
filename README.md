...
預設使用 `free` 模組的自由美術,免原版即可遊玩。但如果你**擁有原版 King's Bounty 的 DOS 遊戲檔**,可以讓畫面換成經典的原版 VGA 美術 —— 而文字仍是繁體中文。

作法:把原版的 `256.CC`、`416.CC`、`KB.EXE` 放進遊戲的資料目錄 (或執行目錄)。啟動時會自動偵測並改用原版美術,UI 與劇情文字維持中文。沒有這些檔案時,維持 free 美術,一切照常。

技術上採型別感知資源解析:圖形優先取原版 DOS 資料,文字/數值優先取我們翻譯的 free 資料。

> 原版遊戲資料受版權保護,**不包含在本專案**,也請勿散布內含原版美術的打包。此功能僅供已擁有正版者本機使用。
